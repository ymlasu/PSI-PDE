function [x,y,t,uRec,vRec,pRec] = mit18086_navierstokes(vis,a1,a2,a3)
% a1: pressure gradient
% a2: advective acceleration
% a3: Laplacian
...
fprintf(', time loop\n--20%%--40%%--60%%--80%%-100%%\n')
uRec = zeros(nt,nx-1,ny);
vRec = zeros(nt,nx,ny-1);
pRec = zeros(nt,nx,ny);
...
for k = 1:nt
   ...
   U = U+a2*dt*(UVy(2:end-1,:)+U2x);
   V = V+a2*dt*(UVx(:,2:end-1)+V2y);
